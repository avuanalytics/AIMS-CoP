...
if ( !is_user_logged_in() ) {
	...
	if ( onesocial_get_option( 'user_login_option' ) ) {
		...
		if ( buddyboss_is_bp_active() && bp_get_signup_allowed() ) {
			...
			<a href="<?php echo bp_get_signup_page(); ?>" class="register header-button animatedClick boss-tooltip yellow-bg" data-target="RegisterBox" data-tooltip="<?php _e( 'Register', 'onesocial' ); ?>"><i class="bb-icon-pencil-square-o"></i> <?php _e( 'Register', 'onesocial' ); ?></a><?php
			//get_template_part( 'template-parts/site-register' );
		}

		?>

		<a href="#" class="login header-button animatedClick boss-tooltip green-bg" data-target="LoginBox" data-tooltip="<?php _e( 'Login', 'onesocial' ); ?>"><i class="bb-icon-exit"></i> <?php _e( 'Login', 'onesocial' ); ?></a><?php
		//get_template_part( 'template-parts/site-login' );
	} else {

		if ( bp_get_signup_allowed() ) {
			?>
			<a href="<?php echo bp_get_signup_page(); ?>" class="header-button boss-tooltip yellow-bg" data-tooltip="<?php _e( 'Register', 'onesocial' ); ?>"><i class="bb-icon-pencil-square-o"></i> <?php _e( 'Register', 'onesocial' ); ?></a>
		<?php } ?>

		<a href="<?php echo wp_login_url(); ?>" class="header-button boss-tooltip green-bg" data-tooltip="<?php _e( 'Login', 'onesocial' ); ?>"><i class="bb-icon-exit"></i> <?php _e( 'Login', 'onesocial' ); ?></a>
